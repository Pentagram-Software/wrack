...
"
  ...
  echo "DEVICE_TOKENS_SECRET. Both the production deploy workflow"
  echo "(.github/workflows/deploy-cloud-functions.yml) and cloudbuild.yaml grant"
  echo "read access to unifiedIngress's runtime service account automatically on"
  echo "every deploy — no manual step needed there. For an ad-hoc deploy via the"
  echo "bare npm scripts instead (bypassing both of those), grant it yourself:"
  echo ""
