...
print_next_steps() {
  ...
  echo "This secret must only ever be read by the healthLegPush Cloud"
  echo "Function's service account (PEN-228, cloud/functions/health-leg.js)"
  echo "— never by the Raspberry Pi or EV3, and never committed to the repo."
  echo "cloudbuild.yaml and .github/workflows/deploy-cloud-functions.yml both"
  echo "grant this automatically on every deploy; to do it by hand instead:"
  echo ""
  echo "  gcloud secrets add-iam-policy-binding ${SECRET_NAME} \\"
  echo "    --member=\"serviceAccount:<healthLegPush-runtime-SA>@${PROJECT_ID}.iam.gserviceaccount.com\" \\"
  echo "    --role=\"roles/secretmanager.secretAccessor\" \\"
  echo "    --project=${PROJECT_ID}"
  echo ""
  echo "healthLegPush reads this secret via cloud/functions/grafana-credentials.js"
  echo "and pushes via an OTLP client (@opentelemetry/exporter-metrics-otlp-http"
  echo "+ -logs-otlp-http) against otlp_endpoint, authenticating with Basic Auth"
  echo "(instance_id : token) for both the metrics and logs exporters."
  echo ""
  echo "Related tickets:"
  echo "  PEN-218 — unified ingress architecture (health-leg design decisions)"
  echo "  PEN-228 — health-leg push function implementation"
