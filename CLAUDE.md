...
| `edge/vision/` | Python / Markdown | Vision/analytics architecture (`README.md`); CatRecognizer (`add-cat-detection-identification`) implements Phase 1 detection + partial Phase 2 identification — `pipeline.py` wires `detection/` (ONNX YOLOv8/v5 wrapper), `events/` (3-frame-confirmation lifecycle state machine), optional `identification/` (embedding + cosine-similarity match), and `telemetry/` (PEN-166's standalone module, `builder.py`/`schemas.py` build/validate the `cat_detection` event) into one per-frame loop; see `edge/vision/README.md`'s "Implementation status" and `DEPLOYMENT.md` for the full export/deploy/benchmark/soak-test hand-off. `pipeline.py`'s `frame_source` is not yet wired to `edge/video-streamer/`'s live camera capture path (`validation/soak_test.py` uses its own OpenCV capture meanwhile) |
