...
allowed-tools: Bash(openspec:*), mcp__claude_ai_Linear__list_projects, mcp__claude_ai_Linear__get_project, mcp__claude_ai_Linear__list_issues, mcp__claude_ai_Linear__save_issue, mcp__claude_ai_Linear__save_status_update
category: Workflow
...
5. **Sync tasks to Linear**
   ...
   c. **Resolve each group's parent issue** — three cases, in priority order (never create a second parent for a group that already has one, however it was recorded):
      ...
      2. **Heading unannotated, but ≥1 task under it is already annotated**: `mcp__claude_ai_Linear__get_issue` does **not** return a `parentId` field (verified against a real annotated task — it returns `project`/`team`/`projectMilestone`/etc. but never `parentId`, even with `includeRelations: true`), so do not try to read a parent off the annotated task. Instead, search `mcp__claude_ai_Linear__list_issues` (scoped to the resolved project) for an existing issue whose **title** matches the group heading (e.g. "1. GCP Infrastructure (fresh build)"). **If found**, reuse its id as `parentId` for step 5.d, and backfill the heading line with that same id as a `<!-- linear:XXX -->` annotation. **If not found**, do not fabricate a new parent issue — leave the heading unannotated, and in step 5.d create that group's remaining tasks without a `parentId` (flat, ungrouped in Linear); note this explicitly in the step 5.e summary so the user can manually group them later if desired.
      3. **Heading unannotated, no tasks under it annotated**: create one Linear issue via `mcp__claude_ai_Linear__save_issue` (omit `id` to create; pass `team` and `project` from step 5.a — both required), titled after the group heading (e.g. "1. GCP Infrastructure (fresh build)"), with a description noting it was generated from `openspec/changes/<name>/tasks.md` for change `<name>` (the repo-relative form — the canonical suffix used consistently in every generated description and in step 5.d's recovery search; never the absolute `changeRoot` path, which differs across machines). Then append the created issue's identifier as a trailing HTML comment on the heading line itself (e.g. `## 1. GCP Infrastructure (fresh build) <!-- linear:PEN-230 -->`).

   d. **For each task without a `<!-- linear:XXX -->` annotation**: full atomicity isn't achievable from a skill (a crash between creating the issue and writing the annotation would otherwise duplicate it on the next run), so first do a light recovery check — search `mcp__claude_ai_Linear__list_issues` (scoped to the resolved project) for an issue whose description already contains the exact string `Task N.M — openspec/changes/<name>/tasks.md` this skill itself writes (repo-relative form, matching step 5.c's create description — not the absolute `changeRoot` path). **If found**, that's a prior crashed attempt — reuse its id instead of creating a new one, and just write the annotation. **If not found**, create one Linear sub-issue via `mcp__claude_ai_Linear__save_issue` (omit `id` to create; pass `team` and `project` from step 5.a, and `parentId` set to the group's issue id resolved in step 5.c — if case 2 found no match, omit `parentId` entirely rather than guessing), titled with the task's own description (drop the `N.M` numeric prefix from the title, but keep the exact task ID in the issue description, e.g. "Task 2.3 — openspec/changes/<name>/tasks.md", so a task can be traced back to its checkbox and vice versa). **Tasks that already have an annotation are skipped entirely** — already synced, don't touch them or their checkbox line.
   ...
   e. **Report what was created**: a short summary (issue count per group) as part of the final output, not the full list of Linear URLs unless the user asks. If step 5.c's case 2 found no matching parent for any group, explicitly call out which groups' tasks were created without a `parentId` (flat, ungrouped in Linear) so the user knows to group them manually if desired.
