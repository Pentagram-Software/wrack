...
**Delivery mechanism: direct synchronous push** (decided 2026-07-12, see the [PEN-218](https://linear.app/pentagram-software/issue/PEN-218/replace-grafana-alloy-direct-push-with-unified-ingress-single-cloud) comment thread). The ingress function calls the health-leg push function directly over HTTP rather than publishing to an intermediate Pub/Sub topic. [ingress.js](../../cloud/functions/ingress.js)'s `pushHealthRecord()` already implements this: a bounded 3s-timeout fetch, `Promise.allSettled` across a batch's health records so one slow call can't hold up the others, and fail-open behavior (a missing `HEALTH_LEG_FUNCTION_URL`, a timeout, or a non-2xx response is logged and dropped, never surfaced to the caller or allowed to block the analytics leg's response).

The Pub/Sub-mediated alternative — a `health` topic symmetric to the analytics leg, consumed by a small pushing function — was considered and explicitly deferred rather than rejected outright: there's no current requirement driving the extra decoupling, and the bounded timeout plus fail-open behavior already keeps a stalled Grafana endpoint from blocking the ingress response. Revisit if the direct call's coupling becomes a real operational problem (e.g. Grafana OTLP gateway latency/availability measurably affecting ingress response times, or a need to retry/buffer health records rather than dropping them).
