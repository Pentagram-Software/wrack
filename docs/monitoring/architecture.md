...
- The analytics side of the ingress (`telemetryIngestion` Cloud Function → BigQuery) is implemented. As of [PEN-216](https://linear.app/pentagram-software/issue/PEN-216/refactor-videotelemetry-to-use-the-new-rpi-telemetry-module), the Pi streamer no longer talks to `telemetryIngestion` directly — `edge/video-streamer/video_telemetry.py`'s `VideoTelemetry` delegates sending to `edge/vision/telemetry`'s `RpiTelemetryCollector`/`RpiTelemetrySender` (PEN-166), which targets `unifiedIngress` with the same per-device auth as every other sender. EV3 analytics is intentionally disabled during the PEN-233 health-only baseline; it must not be restored until PEN-235 fixes the BigQuery insert failure and verifies controller responsiveness.
