...
* The identity-token client (`google-auth-library`'s `GoogleAuth.getIdTokenClient(audience)`, scoped to the health-leg function's URL) is created **once and cached at module scope**, the same lazy-singleton pattern `ingress.js` already uses for `_secretClient`/`_getSecretClient()` — not constructed or re-fetched inside `pushHealthRecord()` itself. `pushHealthRecords()` calls up to 20 `pushHealthRecord()`s concurrently per chunk; acquiring a fresh token per call would mean up to 20 concurrent metadata-server round-trips per chunk, happening *before* the 3s `AbortSignal.timeout()` that only wraps the OTLP push itself — that would blow past the ~7s worst-case bound above, which assumes each record's total cost tops out at the fetch timeout. The cached client's underlying token is refreshed automatically by the auth library ahead of expiry, so after the first acquisition, attaching it to a request (`Authorization: Bearer <token>`) is an in-memory read, not a network call.
