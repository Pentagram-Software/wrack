...
* The identity-token client (`google-auth-library`'s `GoogleAuth.getIdTokenClient(audience)`, scoped to the health-leg function's URL) is cached at module scope — but **the cached value must be the in-flight promise itself, not the awaited/resolved client**. `getIdTokenClient()` is async, unlike `_secretClient`'s plain `new SecretManagerServiceClient()` (a synchronous constructor with no I/O, so `ingress.js`'s existing check-then-assign pattern for it is race-free). A "check if cached, `await` the fetch, then assign" version of that same pattern applied to an async call is *not* race-free: on a cold instance, `pushHealthRecords()` fans out up to 20 concurrent `pushHealthRecord()`s per chunk, and all 20 can observe an empty cache before the first one finishes, each independently hitting the metadata server. Memoizing the promise (`_idTokenClientPromise ||= auth.getIdTokenClient(audience)`, returned and `await`-ed by every caller) fixes this: the promise is assigned synchronously on the first call, before any `await`, so concurrent callers share it instead of racing to fill the cache. Once resolved, the client's underlying token is refreshed automatically by the auth library ahead of expiry, so steady-state token attachment is an in-memory read, not a network call — it's only the cold-start acquisition that needs this care, not every subsequent call.

This needs to land as part of [PEN-228](https://linear.app/pentagram-software/issue/PEN-228/implement-health-leg-push-function-otlp-push-to-grafana-cloud)'s scope — both the IAM grant/deploy flag on the receiving side and the cached identity-token client on the calling side — not as a follow-up hardening pass after the endpoint is already live.
