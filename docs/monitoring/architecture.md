...
**Delivery mechanism: direct synchronous push** (decided 2026-07-12, see the [PEN-218](https://linear.app/pentagram-software/issue/PEN-218/replace-grafana-alloy-direct-push-with-unified-ingress-single-cloud) comment thread). The ingress function calls the health-leg push function directly over HTTP rather than publishing to an intermediate Pub/Sub topic. [ingress.js](../../cloud/functions/ingress.js)'s `pushHealthRecord()` already implements this: a bounded 3s-timeout fetch per health record, run concurrently via `Promise.allSettled` so multiple health records in one batch don't serialize against each other, and fail-open behavior (a missing `HEALTH_LEG_FUNCTION_URL`, a timeout, or a non-2xx response is logged and dropped, never surfaced as an error to the caller). This health-leg await happens **before** the analytics leg runs — it's sequential, not parallel — so it adds latency to the overall ingress response; the timeout bounds that delay, it doesn't eliminate it.

The Pub/Sub-mediated alternative — a `health` topic symmetric to the analytics leg, consumed by a small pushing function — was considered and explicitly deferred rather than rejected outright: there's no current requirement driving the extra decoupling, and the 3s timeout plus fail-open behavior bounds how long a stalled Grafana endpoint can delay the ingress response, even though it doesn't remove that coupling — every request carrying health records still waits on the health leg before the analytics leg even starts. Revisit if that coupling becomes a real operational problem (e.g. Grafana OTLP gateway latency/availability measurably affecting ingress response times, or a need to retry/buffer health records rather than dropping them).
