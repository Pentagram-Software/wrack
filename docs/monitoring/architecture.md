...
### The health leg (open — not yet finalized)

Two options are on the table in the PEN-218 comment thread, and neither has been committed to yet:
...
|---|---|---|
| EV3 | HTTPS POST + per-device token, `type=health` (heartbeat/battery/device status) → unified ingress → health leg → Grafana Cloud | HTTPS POST + per-device token, `type=event` → unified ingress → analytics topic → BigQuery insert fn (PEN-219) → BigQuery |
...
- The unified ingress function itself, the analytics/health Pub/Sub split, and the health-leg push function do not exist yet, beyond PEN-219 covering the analytics leg.
