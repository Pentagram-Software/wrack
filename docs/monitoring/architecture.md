...
|---|---|---|
| EV3 | HTTPS POST + per-device token, `type=health` (liveness heartbeat; battery is added by PEN-234) → unified ingress → health leg (OTLP push) → Grafana Cloud | **Temporarily disabled (PEN-233):** command, motor, device-status, and other analytics records remain off until the safe restoration work in PEN-235 |
