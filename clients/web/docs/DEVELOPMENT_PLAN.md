...
| P1.3 | Export JSON from Material Theme Builder | Designer | P0.4 |
| P1.3a | Create token transformation script | Developer | P1.2 |
| P1.3b | Import JSON and run transform to generate color tokens | Developer | P1.3, P1.3a |
| P1.4 | Define typography, spacing, and elevation tokens | Developer | P1.2 |
| P1.5 | Create `ThemeProvider` component | Developer | P1.3b, P1.4 |
...
- [ ] Material Theme Builder JSON exported and saved to repo
- [ ] Token transformation script created (`npm run tokens:transform`)
- [ ] Design tokens generated from JSON
...
│   ├── colors.generated.ts       # ← Auto-generated by npm run tokens:transform
│   ├── colors.ts                 # Manual extensions/overrides
│   ├── index.ts
│   ├── typography.ts
│   ├── spacing.ts
│   ├── radius.ts
...
scripts/
└── transform-tokens.ts          # JSON → TypeScript transformer
```

#### P1.2a: Initial Token Setup (Material Theme Builder JSON Export)

1. Go to https://m3.material.io/theme-builder
2. Select your primary brand color
3. Customize light/dark schemes as needed
4. Click "Export" → "Material Theme (JSON)"
5. Save the downloaded file as `src/design-system/tokens/exported/material-theme.json`
6. Run `npm run tokens:transform` to generate TypeScript tokens
...
## 8. Phase 5: Figma Integration (Optional / Future)

> **Note**: This phase is optional for the first iteration. The manual Material Theme Builder → JSON export workflow established in Phase 1 provides sufficient design-to-code synchronization for initial adoption. Figma integration can be added later once the design system foundation is stable.

### 8.1 Objectives

- (Optional) Establish Figma as a design workspace using the same tokens
- (Optional) Set up bi-directional sync with MUI Sync Plugin
- Create design handoff documentation

### 8.2 Current Workflow (First Iteration)

For the first iteration, the design-to-code workflow is:

```
Material Theme Builder (Web) → Export JSON → Manual copy to repo → Transform to TypeScript
```

This is intentionally simple and provides full visibility into token changes.

### 8.3 Future Tasks (When Ready for Figma Integration)

| Task | Description | Owner | Dependencies |
|------|-------------|-------|--------------|
| P5.1 | Import Material Theme Builder tokens into Figma Variables | Designer | P4 complete |
| P5.2 | Purchase/set up MUI Design Kit for Figma | Designer | Budget approval |
| P5.3 | Configure Figma Variables matching code tokens | Designer | P5.1, P5.2 |
| P5.4 | (Optional) Install MUI Sync Plugin in Figma | Designer | P5.3 |
| P5.5 | Test automated sync workflow | Designer/Dev | P5.4 |
| P5.6 | Document design-to-code process | Designer/Dev | P5.5 |
| P5.7 | Train team on workflow | Designer/Dev | P5.6 |
