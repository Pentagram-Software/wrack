    ...
    public static func parse(from data: Data) -> FrameStartPacket? {
        ...
        if data.count == 35 {            // 64-bit format (Raspberry Pi 64-bit, primary)
            return body.withUnsafeBytes { ptr -> FrameStartPacket? in
                ...
                let frameID     = UInt64(littleEndian: ptr.load(fromByteOffset:  0, as: UInt64.self))
                let frameSize   = UInt64(littleEndian: ptr.load(fromByteOffset:  8, as: UInt64.self))
                let chunkCount  = UInt64(littleEndian: ptr.load(fromByteOffset: 16, as: UInt64.self))
            ...
            return body.withUnsafeBytes { ptr -> FrameStartPacket? in
                ...
                let frameID     = UInt64(UInt32(littleEndian: ptr.load(fromByteOffset: 0, as: UInt32.self)))
                let frameSize   = UInt64(UInt32(littleEndian: ptr.load(fromByteOffset: 4, as: UInt32.self)))
                let chunkCount  = UInt64(UInt32(littleEndian: ptr.load(fromByteOffset: 8, as: UInt32.self)))
                return FrameStartPacket(frameID: frameID, frameSize: frameSize, chunkCount: chunkCount)
            }
        }
    ...
    public static func parse(from data: Data) -> ChunkPacket? {
        ...
        if data.count >= header64Size {        // Try 64-bit first (primary)
            let parsed: ChunkPacket? = data.dropFirst(marker.count).withUnsafeBytes { ptr in
                ...
                let frameID    = UInt64(littleEndian: ptr.load(fromByteOffset: 0, as: UInt64.self))
                let chunkIndex = UInt64(littleEndian: ptr.load(fromByteOffset: 8, as: UInt64.self))
        ...
        if data.count >= header32Size {        // 32-bit fallback
            return data.dropFirst(marker.count).withUnsafeBytes { ptr in
                ...
                let frameID    = UInt64(UInt32(littleEndian: ptr.load(fromByteOffset: 0, as: UInt32.self)))
                let chunkIndex = UInt64(UInt32(littleEndian: ptr.load(fromByteOffset: 4, as: UInt32.self)))
